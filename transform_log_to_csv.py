...
def get_proba_per_axis_repartition_of_predictions(table, axis, slices):
    axis_proba = (table[(table.axis == axis) & (table.pred > 0) & (pd.Series([x in slices for x in table.slice]))].shape[0]) / float(len(slices))
    axis_proba = round(axis_proba, 2)
    return axis_proba


def get_proba_per_axis_mean(table, axis, slices):
    axis_proba = table[(table.axis == axis) & (pd.Series([x in slices for x in table.slice]))]["pred"].mean()
    axis_proba = round(axis_proba, 2)
    return axis_proba


def get_proba_per_axis_median(table, axis, slices):
    axis_proba = table[(table.axis == axis) & (pd.Series([x in slices for x in table.slice]))]["pred"].median()
    axis_proba = round(axis_proba, 2)
    return axis_proba
